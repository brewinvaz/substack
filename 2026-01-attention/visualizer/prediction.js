...
const HIDDEN_STATES = [
    ...
    [0.25, 0.82, 0.15, 0.55],   // the - article
...
function updateDistributionInsight() {
    ...
    if (temperature < 0.5) {
        text.textContent = 'Very low temperature - distribution is extremely peaked. Almost always selects "Paris".';
...
// Render the alignment visualization showing vocab vectors and dot products
function renderAlignmentVisualization() {
    ...
    // Create hidden state display
    const hiddenStateRow = document.createElement('div');
    hiddenStateRow.className = 'alignment-row hidden-state-row';
    hiddenStateRow.innerHTML = `
        <div class="alignment-label">Hidden State (h)</div>
        <div class="alignment-vector">${formatVector(PREDICTION_HIDDEN_STATE)}</div>
        <div class="alignment-score"></div>
    `;
    container.appendChild(hiddenStateRow);

    setTimeout(() => {
        hiddenStateRow.classList.add('visible');
    }, 100 / speed);

    // Create vocab vector rows with animated dot product
    const vocabEntries = Object.entries(VOCAB_VECTORS);
    vocabEntries.forEach(([token, vec], i) => {
        const row = document.createElement('div');
        row.className = 'alignment-row vocab-row';

        const score = dotProduct(PREDICTION_HIDDEN_STATE, vec);
        const maxScore = dotProduct(PREDICTION_HIDDEN_STATE, VOCAB_VECTORS['Paris']);
        const alignment = (score / maxScore) * 100;

        row.innerHTML = `
            <div class="alignment-label">${token}</div>
            <div class="alignment-vector">${formatVector(vec)}</div>
            <div class="alignment-score-container">
                <div class="alignment-meter">
                    <div class="alignment-fill ${token === 'Paris' ? 'high' : token === 'pizza' ? 'low' : 'medium'}"
                         style="--target-width: ${Math.max(0, alignment)}%"></div>
                </div>
                <span class="alignment-value">${score.toFixed(2)}</span>
            </div>
        `;
        ...
        setTimeout(() => {
            ...
            const fill = row.querySelector('.alignment-fill');
            if (fill) {
    ...
    // Add insight
    setTimeout(() => {
        const insight = document.getElementById('alignmentInsight');
        if (insight) {
            insight.classList.add('visible');
        }
    }, (400 + vocabEntries.length * 200 + 300) / speed);
}
